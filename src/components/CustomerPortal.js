...
class ApplyToken extends React.Component {
  constructor(props) {
    super(props);
    this.state = { token: '', busy: false, error: null, result: null };
  ...
  async apply(e) {
    ...
    this.setState({ busy: true, error: null, result: null });
    try {
      const result = await amiFetch(
        `/meters/${encodeURIComponent(this.props.serial)}/apply-token`, {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ token: this.state.token }),
        });
      this.setState({ busy: false, result, token: '' });
  ...
  render() {
    ...
    return (
      <div className="card">
        <h2>Load Token on Meter — {this.props.serial}</h2>
        <p className="muted">Enter a purchased 20-digit token to load the credit
          onto your meter (same as typing it on the meter keypad).</p>
        {s.error && <div className="error">{s.error}</div>}
        {s.result && (
          <div className="card" style={{ background: '#eef8f0' }}>
            <b>Credit loaded!</b> +{s.result.kwhAdded} kWh —
            new balance <b>{s.result.newBalanceKwh} kWh</b>.
          </div>
        )}
        <form onSubmit={this.apply}>
          <div className="controls">
            <label>Token (20 digits, dashes optional)
              <input value={s.token} required inputMode="numeric"
                placeholder="0000-0000-0000-0000-0000"
                onChange={(e) => this.setState({ token: e.target.value })} />
            </label>
            <button className="btn" type="submit" disabled={s.busy}>
              {s.busy ? 'Applying…' : 'Load Token'}
            </button>
          </div>
        </form>
      </div>
        ...
        <main className="page cust-page">
          ...
          {tab === 'buy' && meter && (
            <React.Fragment>
              ...
              <ApplyToken serial={meter.meter_serial} onApplied={this.load} />
            </React.Fragment>
          )}
                  ...
                  <tbody>
                    {txns.transactions.map((t) => (
                      <tr key={t.txn_id}>
                        ...
                        <td><span className={'badge ' + (t.status === 'applied' ? 'online' : 'auto_registered')}>{t.status}</span>
                          {t.status === 'paid' && (
                            <button className="btn secondary" style={{ padding: '2px 8px', marginLeft: 6, fontSize: 11 }}
                              onClick={async () => {
                                try {
                                  await amiFetch(`/meters/${encodeURIComponent(serial)}/apply-token`, {
                                    method: 'POST', headers: { 'Content-Type': 'application/json' },
